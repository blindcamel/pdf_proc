...
{ (1, 1):["CompanyA", "456123abc", "INV456"], (1, 2):["CompanyA", "456785swz", "INV457"] }
...
http://localhost:8000 "endpoints": { "POST /upload": "Upload and process a new PDF file", "POST /process-file": "Process an existing file from the input directory", "POST /process-all": "Process all files in /uploads", "GET /list-files": "List all PDF files in the input directory", "GET /debug-paths": "debug paths", "GET /processing-status": "List all", "GET /processing-status/{filename}": "List one", "DELETE /processing-status": "Clear all", "DELETE /processing-status/{filename}": "Clear one", "GET /": "This information", }
...
#get processing status curl -X GET http://localhost:8000/processing-status/TPW98705.pdf
